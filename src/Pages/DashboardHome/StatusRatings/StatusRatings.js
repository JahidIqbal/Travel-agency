...
const StatusRatings = () => {
  ...
  const handleStatus = (_id, pd) => {
    ...
    fetch(`https://travel-agency-server-ru04.onrender.com/ratings/${_id}`, {
      method: "PUT",
  ...
  const handleRemove = (id) => {
    const url = `https://travel-agency-server-ru04.onrender.com/ratings/${id}`;
    const isReady = window.confirm(
      "are you sure you want to delete this ratings?"
    );
    if (isReady) {
      fetch(url, {
        method: "DELETE",
        ...
        <table className="table table-borderless  text-bold">
          <thead>
            <tr>
              ...
              <th scope="col">
                Testimonials list <i className="far fa-clipboard"></i>
              </th>
